...
class Vtiger_InventoryPDFController {
	...
	function buildSummaryModel() {
		...
		$summaryModel->set(getTranslatedString("الاجمالي الفرعي ", $this->moduleName), $this->formatPrice($netTotal));
		...
		if($final_details['taxtype'] == 'group') {
			...
			$summaryModel->set(getTranslatedString("الضرائب  : 5%", $this->moduleName), $this->formatPrice($final_details['tax_totalamount']));
		}
		...
		$summaryModel->set(getTranslatedString("الاجمالي", $this->moduleName), $this->formatPrice($final_details['grandTotal'])); // TODO add currency string
	...
	function buildHeaderModel() {
		...
		$headerModel->set('vatid', $this->getvatid());
	...
	function getvatid(){
		global $adb;
		$vatid="";
		$result = $adb->pquery("SELECT vatid FROM vtiger_organizationdetails", array());
		$num_rows = $adb->num_rows($result);
		if($num_rows) {
          $vatid= $adb->query_result($result, 0, 'vatid');
		}
		return $vatid;
	...
	function buildHeaderModelColumnLeft() {
		...
		if($num_rows) {
			...
			if(!empty($resultrow['website']))	$additionalCompanyInfo[]= "\nWebsite :".$resultrow['website']." \n\nZMN ALHOLOOL  est \nAlinma bank ACCOUNT NUMBER 68201297338000 \n Ipan SA0305000068201297338000\nZMN ALHOLOOL  est \nNCB ACCOUNT NUMBER 12300000371905 \nIpan SA2310000012300000371905
";
                        if(!empty($resultrow['vatid']))         $additionalCompanyInfo[]= "\nVAT ID :". $resultrow['vatid']; 
	...
	function buildHeaderModelColumnCenter() {
        ...
        $recevidcu = $this->focusColumnValue('cf_944');
		...
		$customerNameLabel ="Customer Name" ;//getTranslatedString('Customer Name', $this->moduleName);
		$contactNameLabel ="Contact Name"; // getTranslatedString('Contact Name', $this->moduleName);
		...
		$modelColumnCenter = array(
				...
				$contactmobil => $Mobile,
				$contactemail => $email,
				$receivedleble => $recevidcu,
		);
		return $modelColumnCenter;
	}

	function buildHeaderModelColumnRight() {
		$issueDateLabel ="Issued Date";// getTranslatedString('Issued Date', $this->moduleName);
		$validDateLabel ="Valid Date";// getTranslatedString('Valid Date', $this->moduleName);
		$Ysername="username";
