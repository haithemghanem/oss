<?php
class Reports_RevenueReportsPrint_View extends Vtiger_View_Controller {
...
public function checkPermission(Vtiger_Request $request) {
    ...
    $frombranch1 = explode(',', $frombranch);
    ...
    $frombranchlebel="";
     ...
     $count1=count($frombranch1);
    if( $count1 > 0){
        for($i=1 ; $i <=  $count1; $i++){
         $formbrochinval .='"'.$frombranch1[$i-1].'"';
         $frombranchlebel.=vtranslate($frombranch1[$i-1],'Invoice');
            if($i < $count1)
            {
              $formbrochinval .=",";
              $frombranchlebel .="_";
    ...
    if( $count > 0){
       ...
       WHERE vtiger_invoice.invoiceid > 0 AND vtiger_crmentity.deleted=0 and  vtiger_invoice.invoicedate !='' and vtiger_invoicecf.cf_950 !='' And  vtiger_invoicecf.cf_948 !='' AND vtiger_invoicecf.cf_954 !='' 
        ";
    ...
    if(!empty($formbrochinval)){
              $Sql.="    AND  vtiger_invoicecf.cf_948 IN ($formbrochinval) ";
      }
     ...
     if(!empty($inval)){
       ...
       $Sql.=" GROUP BY vtiger_invoice.invoiceid order by vtiger_invoice.invoice_no   LIMIT 0, 500000";
        ...
        //echo "goooood;"
         ...
         $result=$adb->pquery($Sql,$paramter);
          if($adb->num_rows($result) > 0)
            {
              ...
              while($roww = $adb->fetch_array($result))
               ...
               <td colspan="3">الاجمالي:</td>
                 ...
                 <td class="wrwarp">'.number_format($sumcoust - $invoice_discount_amount_total,2).'</td>
                 ...
                 <td colspan="2">'.number_format($seroffice_totoal,2).'</td>
                 </tr>
          ...
          if(!empty($excel) && $excel =="1"){
              $file_name="تقرير الايرادات-".$datalebel."_.xls";
...
<html>
    <head>
        ...
        <title>تقرير الايرادات-<?php echo $datalebel;?></title>
        ...
        <table width="100%" border="0" cellpadding="5" cellspacing="0" align="center">
            <tr class="reportPrintHeader">
                ...
                <td  class="right"  style="border:0px solid #000000;padding-left: 18%;" valign="top">
                    <h4 style="color:#CCCCCC">تقرير الايرادات</h4>
                </td>
            </tr>
            <tr><td colspan="2">
                <div style="text-align: right">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td><?php  echo $frombranchlebel ?></td>
