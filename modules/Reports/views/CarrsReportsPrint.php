...
class Reports_CarrsReportsPrint_View extends Vtiger_View_Controller {
//CarrsReportsPrint.php
	...
	function process(Vtiger_request $request) {
        ...
        if ( $date  != '' &&  $dateto !='0'){
         ...
         if ( $Carriersid  != '' &&  $Carriersid !='0'){

              $curresql = "  AND  vtiger_invoicecf.cf_954=$Carriersid ";
         }
...
(CASE WHEN vtiger_invoicecf.cf_954 NOT LIKE '' THEN (CASE WHEN trim(vtiger_vendorRelInvoice955.vendorname) NOT LIKE '' THEN trim(vtiger_vendorRelInvoice955.vendorname) ELSE '' END) ELSE '' END) AS 'Invoice_Name_Carrier'
   ...
   if(!empty($datasql)){
    ...
    if(!empty($curresql)){
      ...
      $Sql.="GROUP BY vtiger_invoice.invoicedate ,vtiger_invoicecf.cf_954  , vtiger_invoicecf.cf_958 ,  vtiger_invoicecf.cf_948 , vtiger_invoicecf.cf_950  order by vtiger_invoice.invoicedate DESC";
		  ...
		  if($adb->num_rows($result) > 0)
		    {
			  ...
			  while($roww = $adb->fetch_array($result))
			   { 
                ...
                $frombranch = $roww['invoice_from_branch']; //Invoice_From_branch

			   	
			   	$data.='<tr>
			   	 <td>'.$sequ.'</td>
			   	 <td>'.$countinv.'</td>
			   	 <td>'.vtranslate($frombranch,'Invoice').'</td>
                 ...
                 <td>'.$numcapan.'</td>
                  ...
                  <td><a target="_blank" href="index.php?module=Reports&view=CarriersReportPrint&frombranch='.$frombranch.'&Carriersid='.$Carriersnid.'&tobranch='.$robranch.'&date='.$invoic_Data.'&capnum='.$numcapan.'"> التفاصيل  </a> </td>
        ...
        <table width="100%" border="0" cellpadding="5" cellspacing="0" align="center">
            <tr class="reportPrintHeader">
                ...
                <td  class="right"  style="border:0px solid #000000;padding-left: 18%;" valign="top">
                    <h4 style="color:#CCCCCC"> تقرير الحملات  </h4>
                </td>
            </tr>
            <tr><td colspan="2">
