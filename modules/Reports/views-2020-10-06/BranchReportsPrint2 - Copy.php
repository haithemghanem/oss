...
//BranchReportsPrint2
	...
	function process(Vtiger_request $request) {
		...
		$Sql="
		 SELECT vtiger_invoice.pre_tax_total AS 'Invoice_Pre_Tax_Total',vtiger_invoicecf.cf_987 AS 'sendername', vtiger_invoicecf.cf_950 AS 'Invoice_To_branch', vtiger_invoicecf.cf_948 AS 'Invoice_From_branch', (CASE WHEN vtiger_invoice.contactid NOT LIKE '' THEN (CASE WHEN trim(CONCAT(vtiger_contactdetailsInvoice.firstname,' ',vtiger_contactdetailsInvoice.lastname)) NOT LIKE '' THEN trim(CONCAT(vtiger_contactdetailsInvoice.firstname,' ',vtiger_contactdetailsInvoice.lastname)) ELSE '' END) ELSE '' END) AS 'Invoice_Contact_Name', (CASE WHEN vtiger_invoicecf.cf_944 NOT LIKE '' THEN (CASE WHEN trim(CONCAT(vtiger_contactdetailsRelInvoice945.firstname,' ',vtiger_contactdetailsRelInvoice945.lastname)) NOT LIKE '' THEN trim(CONCAT(vtiger_contactdetailsRelInvoice945.firstname,' ',vtiger_contactdetailsRelInvoice945.lastname)) ELSE '' END) ELSE '' END) AS 'Invoice_Received_Name', vtiger_invoice.invoicedate AS 'Invoice_Invoice_Date', vtiger_invoice.invoice_no AS 'Invoice_Invoice_No', CASE WHEN (vtiger_productsInvoice.productname NOT LIKE '') THEN vtiger_productsInvoice.productname ELSE vtiger_serviceInvoice.servicename END AS 'Invoice_Item_Name', vtiger_inventoryproductreltmpInvoice.quantity AS 'Invoice_Quantity', CASE WHEN vtiger_currency_infoInvoice.id = vtiger_usersInvoice.currency_id THEN vtiger_inventoryproductreltmpInvoice.listprice/vtiger_currency_infoInvoice.conversion_rate ELSE vtiger_inventoryproductreltmpInvoice.listprice/vtiger_invoice.conversion_rate END AS 'Invoice_List_Price', vtiger_inventoryproductreltmpInvoice.comment AS 'Invoice_Item_Comment', vtiger_invoice.subject AS 'Invoice_Subject',
    ...
    (( vtiger_invoicecf.cf_948 = '$frombranch' and vtiger_invoice.invoicedate = '$date' and vtiger_invoicecf.cf_950 ='$tobranch' AND vtiger_invoicecf.cf_958='$capnum' ) ) GROUP BY vtiger_invoice.invoice_no order by vtiger_invoice.invoice_no   LIMIT 0, 500 
		";
		  ...
		  if($adb->num_rows($result) > 0)
		    {
			  ...
			  while($roww = $adb->fetch_array($result))
			   ...
			   <td colspan="5">الاجمالي الكلي</td>
			   	 <td>'.intval($sumquantity).' </td>
			   	 <td>'.intval($sumcoust).'</td>
			   	 <td>'.intval($sumtax).'</td>
			   	 <td>'.intval($sumothercoust).'</td>
			   	 <td>'.intval($sumgrandtotal).'</td>
