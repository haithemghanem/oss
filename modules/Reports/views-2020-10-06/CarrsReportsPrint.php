...
(CASE WHEN vtiger_invoicecf.cf_954 NOT LIKE '' THEN (CASE WHEN trim(vtiger_vendorRelInvoice955.vendorname) NOT LIKE '' THEN trim(vtiger_vendorRelInvoice955.vendorname) ELSE '' END) ELSE '' END) AS 'Invoice_Name_Carrier'
   ...
   if(!empty($datasql)){
    ...
    if(!empty($curresql)){
      ...
      $Sql.=" GROUP BY vtiger_invoice.invoicedate , vtiger_invoicecf.cf_954  , vtiger_invoicecf.cf_958 order by vtiger_invoice.invoicedate DESC
		";
			   ...
			   { 
                ...
                $Carriersnid = $roww['carriersnid'];
                 ...
                 <td>'.$numcapan.'</td>
                  ...
                  <td><a target="_blank" href="index.php?module=Reports&view=CarriersReportPrint&Carriersid='.$Carriersnid.'&tobranch='.$robranch.'&date='.$invoic_Data.'&capnum='.$numcapan.'"> التفاصيل  </a> </td>
