<?php
//ContactsReport Print

class Reports_ContactsReportPrint_View extends Vtiger_View_Controller {
    ...
    //index.php?module=Reports&view=CarriersReportPrint&Carriersid=3275&tobranch=Mahail Asir&date=2019-07-13&campnumber=1
    ...
    function process(Vtiger_request $request) {
        ...
        global $adb;
        $inval = "";
        $id = $request->get('contactname');
        $companyname="";

    
         $quer="SELECT  CONCAT(vtiger_contactdetails.firstname,' ',vtiger_contactdetails.lastname) AS 'contactname'  FROM vtiger_contactdetails
                                INNER JOIN vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid AND  vtiger_crmentity.deleted=0   WHERE  vtiger_contactdetails.contactid=? LIMIT 1";
                           $paramter=array($id);
                             $result=$adb->pquery($quer,$paramter);
                             if($adb->num_rows($result) > 0)
                              {
                                $companyname= $adb->query_result($result ,0,'contactname');
                              }
        ...
        <table width="100%" border="0" cellpadding="5" cellspacing="0" align="center">
            <tr class="reportPrintHeader">
                ...
                <td  class="right"  style="border:0px solid #000000;padding-left: 18%;" valign="top">
                    <h4 style="color:#CCCCCC"> تقرير العملاء</h4>
                </td>
            </tr>
            <tr><td colspan="2">
