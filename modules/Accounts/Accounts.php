...
class Accounts extends CRMEntity {
	...
	*/
	function get_invoices($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
				vtiger_crmentity.*, vtiger_invoice.*,vtiger_invoicecf.*, vtiger_account.accountname, vtiger_salesorder.subject AS salessubject
				FROM vtiger_invoice
		...
		if(!empty($entityIds)){
			$query .= " OR vtiger_invoice.contactid IN (".$entityIds.") OR vtiger_invoicecf.cf_944 IN (".$entityIds.") )";
		} else {
			$query .= ")";
		}
