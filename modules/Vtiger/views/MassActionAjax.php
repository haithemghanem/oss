...
class Vtiger_MassActionAjax_View extends Vtiger_IndexAjax_View {
	function __construct() {
		...
		$this->exposeMethod('showMandoubForm');
		$this->exposeMethod('showMandoubStausForm');
	...
	function showSendSMSForm(Vtiger_Request $request) {
	 ...
	 /**
	 * Function shows form that will chaing Mandoub
	 * @param Vtiger_Request $request
	 */
	function showMandoubForm(Vtiger_Request $request) {
		$sourceModule = $request->getModule();
		$moduleName = 'SMSNotifier';
		$selectedIds = $this->getRecordsListFromRequest($request);
		$excludedIds = $request->get('excluded_ids');
        ...
        $db = PearDatabase::getInstance();
        $query ="SELECT vtiger_mandub.mandubid, vtiger_mandub.name FROM vtiger_mandub 
					INNER JOIN vtiger_crmentity on  vtiger_crmentity.crmid = vtiger_mandub.mandubid
					WHERE vtiger_crmentity.deleted =0";
        $result = $db->pquery($query,array());
        $num_rows = $db->num_rows($result);
         $mundub_record;
         while($roww = $db->fetch_array($result))
           { 
           	$mundub_record[]= array('name'=> $roww['name'] , 'mandubid' => $roww['mandubid'] );;
          }
          
        $viewer->assign('MUNDUB_RECORD',$mundub_record);
        $searchKey = $request->get('search_key');
        $searchValue = $request->get('search_value');
		$operator = $request->get('operator');
        if(!empty($operator)) {
		...
		echo $viewer->view('SendMandoubForm.tpl', $moduleName, true);
	}
	
	
	/**
	 * Function shows form that will chaing Mandoub Staus
	 * @param Vtiger_Request $request
	 */
	function showMandoubStausForm(Vtiger_Request $request) {
		$sourceModule = $request->getModule();
		$moduleName = 'SMSNotifier';
		$selectedIds = $this->getRecordsListFromRequest($request);
		$excludedIds = $request->get('excluded_ids');
		...
		echo $viewer->view('SendMandoubStausForm.tpl', $moduleName, true);
	}
