...
class CheckDuplicateHandler extends VTEventHandler {
	...
	public function triggerCheckDuplicateHandler($entityData) {
		...
		if($moduleName =="Invoice"){
            ...
            $pre_tax_total = floatval($fieldValues['pre_tax_total']);
            $difrent =($pre_tax_total + ($pre_tax_total * 0.15)) - $total;
            if($difrent > 0.1  && $total > 1){
            	$exception = new DuplicateException("عذرا , لم يتم الحفظ تاكد من الاجمالي الكلي ثم احفظ");
			...
			if (count($uniqueFields) > 0) {
				...
				if ($checkDuplicates) {
					...
					if (count($conditions) > 0) {
						$conditionGlue = ($moduleModel->duplicateCondition == 'all') ? 'AND' : 'AND';
