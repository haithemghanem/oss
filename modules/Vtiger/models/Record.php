...
class Vtiger_Record_Model extends Vtiger_Base_Model {
	 ...
	 */
	public static function getSearchResult($searchKey, $module=false , $tocity=false) {
		...
		if(!empty($tocity)  && $module == "Contacts"){

            $query = 'SELECT vtiger_crmentity.label, vtiger_crmentity.crmid, vtiger_crmentity.setype, vtiger_crmentity.createdtime FROM vtiger_crmentity
             LEFT join   vtiger_contactscf on  vtiger_contactscf.contactid = vtiger_crmentity.crmid
             WHERE label LIKE ? AND (vtiger_contactscf.cf_972 = ? Or vtiger_contactscf.cf_972 = null) AND vtiger_crmentity.deleted = 0 ';
              $params = array("%$searchKey%" , $tocity);

		}elseif(!empty($tocity)  && $module == "PriceBooks"){

			$query = 'SELECT vtiger_crmentity.label, vtiger_crmentity.crmid, vtiger_crmentity.setype, vtiger_crmentity.createdtime FROM vtiger_crmentity
             LEFT join   vtiger_pricebookcf on  vtiger_pricebookcf.pricebookid = vtiger_crmentity.crmid
             WHERE label LIKE ? AND (vtiger_pricebookcf.cf_968 = ? Or vtiger_pricebookcf.cf_968 = null) AND vtiger_crmentity.deleted = 0 ';
              $params = array("%$searchKey%" , $tocity);
