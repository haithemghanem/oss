...
class Vtiger_DashBoard_Model extends Vtiger_Base_Model {
	...
	public function addTab($tabName){
		...
		$default = $db->pquery("SELECT isdefault FROM vtiger_dashboard_tabs WHERE userid=? AND isdefault=1 ",array($currentUser->getId()));
		
		$numRow = $db->num_rows($default);
		if ($numRows > 0) {
			$isdefault=0;
		}
		
	    
		$query = "INSERT INTO vtiger_dashboard_tabs(tabname, userid,sequence,isdefault) VALUES(?,?,?,?)";
		$db->pquery($query,array($tabName,$currentUser->getId(),$sequence,$isdefault));
