...
class Vtiger_Module_Model extends Vtiger_Module {
	 ...
	 */
	public function searchRecord($searchValue, $parentId=false, $parentModule=false, $relatedModule=false ,$tocity=false) {
			$searchFields = array('crmid','label','setype');
		if(!empty($searchValue) && empty($parentId) && empty($parentModule)) {
			$matchingRecords = Vtiger_Record_Model::getSearchResult($searchValue, $this->getName(),$tocity);
...
//						3 => 'LBL_PREFER_VTIGER_RECORD',
	...
	public function getModuleIcon() {
		...
		if($moduleName == "Mandub"){
		    $moduleIcon = "<i class='vicon-vendors3' title='$title'></i>";
		}
