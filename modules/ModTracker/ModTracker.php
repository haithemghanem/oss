...
class ModTracker {
    ...
    */
    ...
    static function trackRelation($sourceModule, $sourceId, $targetModule, $targetId, $type) {
        ...
        //this cod create by haitham Ghanem 2018-03-03
        $currentUserMode = Users_Record_Model::getCurrentUserModel();
        $currentUserModel=$currentUserMode->getId();
        
        $selectcreatuser="SELECT smcreatorid From vtiger_crmentity  WHERE crmid=?";
        $parmtere=array($sourceId);
       ...
       if($adb->num_rows($result) >= 1)
       {
        ...
        $adb->pquery('INSERT INTO vtiger_modtracker_basic(id, crmid, module, whodid, changedon, status) VALUES(?,?,?,?,?,?)',
                array($id , $sourceId, $sourceModule, $currentUserModel, $currentTime, $type));
