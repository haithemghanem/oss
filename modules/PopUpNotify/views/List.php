...
//chdir('../../');
/*111*/
class PopUpNotify_List_View extends Vtiger_Index_View {
...
<div style="padding: 40px;">
  ...
  <div class="col-md-12">
     ...
     <div  class="col-md-6" > <h2><?php echo vtranslate('Popup Notification:','PopUpNotify')?></h2>  </div>
      <div  class="col-md-6" >
      <button type="button" class="btn btn-default pull-right" title="Create New" onclick="spAdd_Click();"> Add New </button>
      <a href="index.php?module=PopUpNotify&view=AllList"  class="btn btn-default pull-right" > View All</a>
      </div>
...
<div id="dlg-pop-editor" title="Popup Notify" style="display:none;">
    <div class="dv-pop-card" id="dv-pop-editor" >
        <div class="dv-col1" style="width: 100%;">
            ...
            <input type="textbox"  class="entry-value" id="pop-entry-date" placeholder="" readonly='true' style="cursor: text;" />
            <br />
...
<link type="text/css" rel="stylesheet" href="/libraries/datatables.net-bs/css/dataTables.bootstrap.min.css" rel="stylesheet">
 ...
 $curr_userid = $_SESSION['authenticated_user_id'];
   // $curr_userid=25;
   // $Select="select pop.*,com.view_stat from tblPopupNotify pop ,tblPopup_Comment com WHERE  owner_id =25 AND  pop.id=popid AND view_stat !=1";           
   
 //$dbquery="select pop.*,COUNT(com.view_stat) as count from tblPopupNotify pop ,tblPopup_Comment com WHERE  owner_id =? AND  pop.id=popid AND view_stat !=1 GROUP BY com.view_stat ORDER by id DESC";
 //$dbquery="select pop.*, COUNT(tblPopup_Comment.view_stat) as count from tblPopupNotify pop LEFT JOIN  tblPopup_Comment ON pop.id=popid AND view_stat !=1 WHERE  pop.owner_id =25  GROUP BY tblPopup_Comment.view_stat ORDER By pop.id";
 $dbquery="select pop.*, COUNT(tblPopup_Comment.popid) as count,COUNT(tblPopupNotify_Users.popid)as usecount from tblPopupNotify pop LEFT JOIN  tblPopup_Comment ON pop.id=popid AND view_stat !=1 LEFT JOIN tblPopupNotify_Users on pop.id =tblPopupNotify_Users.popid   WHERE  pop.owner_id =?  GROUP BY tblPopup_Comment.popid , tblPopupNotify_Users.popid ORDER By pop.id DESC limit 20";

 
$result = $adb->pquery($dbquery, array($curr_userid));
$countm = $adb->num_rows($result);
...
for($i=0; $i < $countm; $i++){
    ...
    $countuser=$adb->query_result($result,$i,'usecount');
    $countucomment=$adb->query_result($result,$i,'count');
    ...
    ';
       if($countuser ==0){
           echo' <td style="background: #aff3ef;">';
       }else{
          echo' <td>';
       }
       echo'
                           
                            <span Style="padding: 5px;" class="sp-tool2 sp-edt2" title="Edit" onclick="spEdit_Click('. $pid .');">
                            <i title="Edit" class="fa fa-pencil"></i>
                            </span>
                            <span Style="padding: 5px;"  class="sp-tool2 sp-del2" title="Delete" onclick="spDelete_Click('. $pid .');">
                            <i title="Unlink" class="fa fa-close"></i>
                            </span>
                            <span  Style="padding: 5px;" class="sp-tool2 sp-notes" onclick="spCommentInbox_Click('. $pid .');" title="Open comment inbox for this popup">
                            <i title="Unlink" class="fa fa-comment"></i>'.$countucomment.'
                            </span>
                            
                            <span Style="padding: 5px;" class="sp-tool2 sp-notes" onclick="ShowUser('. $pid .');" title="Show and edite Users">
                            <i title="Show and edite Users" class="fa fa-users"></i>
                            </span>
