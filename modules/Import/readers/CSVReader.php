...
class Import_CSVReader_Reader extends Import_FileReader_Reader {
	...
	public function addRecordsToDB($filePath,$columnNames) {
		...
		$query = 'LOAD DATA LOCAL INFILE "'.$filePath.'" INTO TABLE '.$tableName.' CHARACTER SET UTF8 FIELDS TERMINATED BY "'.$delimiter.'" OPTIONALLY ENCLOSED BY "\"" LINES TERMINATED BY "\n"';
		if($this->hasHeader()){
