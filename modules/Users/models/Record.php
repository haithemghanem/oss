...
class Users_Record_Model extends Vtiger_Record_Model {
    ...
    function addDashboardTabs(){
        ...
        foreach($tabs as $seq => $tabName){
          ...
          //   $ins="INSERT INTO testtb(id,action) VALUES (null,?)";
		  //   $action=$tabName ."-iduser-".$this->getId()."sequ".$seq."isdef".$isDefault;
		  //   $db->pquery($ins,array($action));
		  // }
        }
    }

    public function checkTabsLimitExceed() {
		$db = PearDatabase::getInstance();
    ...
    /**
	 * Function to get full to all user active 
	 */
	public function usersfullname(){
		$db = PearDatabase::getInstance();
		$sql ="SELECT id , CONCAT(first_name,' ',last_name)as fullname FROM vtiger_users WHERE status=?";
		$result = $db->pquery($sql, array('ACTIVE'));
		$noUsers = $db->num_rows($result);
		$users = array();
		if($noUsers > 0) {
			for($i=0; $i<$noUsers; ++$i) {
				$userId = $db->query_result($result, $i, 'id');
				$fullname = $db->query_result($result, $i, 'fullname');
