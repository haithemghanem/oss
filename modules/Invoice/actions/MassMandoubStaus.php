...
class Invoice_MassMandoubStaus_Action extends Vtiger_Mass_Action {
	...
	public function process(Vtiger_Request $request) {
       ...
       if(!empty($mandubField)){
             $query= "UPDATE vtiger_invoicecf SET cf_1012='$mandubField' WHERE invoiceid IN($recordID)";
		...
		if(!empty($mandubField)) {
			...
			//$response->setResult(array('id' => $mandubField, 'statusdetails' => 'Sucessfully Update='.$update));
		}
		return $response;
	}
}
