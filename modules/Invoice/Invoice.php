	...
	*/
	...
	function save_module($module) {
		...
		if(isset($this->_recurring_mode) && $this->_recurring_mode == 'recurringinvoice_from_so' && isset($this->_salesorderid) && $this->_salesorderid!='') {
			...
			if($_REQUEST['action'] != 'InvoiceAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW'
				...
				if(empty($this->column_fields['invoicedate']) ||  $this->column_fields['invoicedate'] == ''  ){
				    
    				$createdate = date('Y-m-d');
    			    $this->column_fields['invoicedate'] = 	$createdate;
                    $query1 = "UPDATE vtiger_invoice SET invoicedate=? WHERE invoiceid=?";
                    $this->db->pquery($query1, array($createdate , $this->id));
		      }
		...
		$update_query = "update vtiger_invoice set currency_id=?, conversion_rate=? , storagevalue=? , deliveryvalue=? where invoiceid=?";

		$update_params = array($this->column_fields['currency_id'], $this->column_fields['conversion_rate'],$this->column_fields['storagevalue'],$this->column_fields['deliveryvalue'] , $this->id);
		$this->db->pquery($update_query, $update_params);
		
		
		if(!empty($this->column_fields['cf_991'])  &&  "Rejected" == $this->column_fields['cf_991']){
		    
          $update_query1 = "update vtiger_invoice set adjustment=0,subtotal=0,total=0,discount_amount=0,s_h_amount=0,pre_tax_total=0,received=0,balance=0,s_h_percent=0  where invoiceid=?"; 
		  $this->db->pquery($update_query1, array($this->id));
         
          $update_query2 = "update vtiger_inventoryproductrel set listprice=0,listprice=0  where id=?";
          $this->db->pquery($update_query2, array($this->id));
	...
	function generateReportsSecQuery($module,$secmodule,$queryPlanner){
		...
		$matrix->setDependency('vtiger_invoice',array('vtiger_crmentityInvoice', "vtiger_currency_info$secmodule",
				...
				'vtiger_invoiceshipads', 'vtiger_inventoryproductrelInvoice', 'vtiger_contactdetailsInvoice','vtiger_contactdetailsRelInvoice945' ,'vtiger_vendorRelInvoice955', 'vtiger_accountInvoice'));
		...
		if ($queryPlanner->requireTable('vtiger_contactdetailsRelInvoice945')) {
			$query .= " left join vtiger_contactdetails as vtiger_contactdetailsRelInvoice945 on vtiger_invoicecf.cf_944 = vtiger_contactdetailsRelInvoice945.contactid";
		}
		if ($queryPlanner->requireTable("vtiger_vendorRelInvoice955")) {
			 $query .= " left join vtiger_vendor as vtiger_vendorRelInvoice955 on vtiger_vendorRelInvoice955.vendorid = vtiger_invoicecf.cf_954";
		}
