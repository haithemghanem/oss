...
class Inventory_Detail_View extends Vtiger_Detail_View {
	...
	function showLineItemDetails(Vtiger_Request $request) {
		...
		$finalDetails['storagevalue'] = $recordModel->get('storagevalue');
		$finalDetails['deliveryvalue'] = $recordModel->get('deliveryvalue');
		...
		$currencyFieldsList = array('adjustment', 'grandTotal', 'hdnSubTotal', 'preTaxTotal', 'tax_totalamount',
									'shtax_totalamount', 'discountTotal_final', 'discount_amount_final', 'shipping_handling_charge','storagevalue','deliveryvalue','totalAfterDiscount', 'deductTaxesTotalAmount');
		...
		for ($i=1; $i<=$productsCount; $i++) {
			...
			$currencyFieldsList = array('taxTotal', 'netPrice', 'listPrice', 'unitPrice', 'productTotal','purchaseCost','margin',
										'discountTotal', 'discount_amount', 'totalAfterDiscount', 'storagevalue','deliveryvalue');
			foreach ($currencyFieldsList as $fieldName) {
