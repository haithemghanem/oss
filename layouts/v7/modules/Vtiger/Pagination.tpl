...
<div class = "{$CLASS_VIEW_ACTION}">
            ...
            <ul class="{$CLASS_VIEW_BASIC_ACTION} dropdown-menu" id="PageJumpDropDown">
                <li>
                    ...
                    <div class="listview-pagejump">
                        ...
                        <button type="button" id="pageToJumpSubmit" class="btn btn-success {$CLASS_VIEW_PAGING_INPUT_SUBMIT} text-center">{vtranslate('GO', $MODULE)} </button>
                    </div>    
                </li>
            </ul>
        {/if}
    ...
    <span class="pageNumbers  pull-right" style="position:relative;top:7px;">
        <span class="pageNumbersText">
            {if $RECORD_COUNT}
            <strong class="pull-right">{$PAGING_MODEL->getRecordStartRange()}</strong>
            <strong>{vtranslate('LBL_to', $MODULE)} </strong>
            <strong class="pull-left">{$PAGING_MODEL->getRecordEndRange()}</strong>{else}
            {/if}
        </span>
