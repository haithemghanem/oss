...
<div class="referencefield-wrapper {if $FIELD_VALUE neq 0} selected {/if}">
    ...
    <div class="input-group">
        ...
        <a href="#" class="clearReferenceSelection {if $FIELD_VALUE eq 0}hide{/if}"> x </a>
            <button  style="    min-width: 28px;background: #F3F3F3;" type="button" name="search" class="input-group-addon relatedPopup btn btn-default " title="{vtranslate('LBL_SELECT', $MODULE)}">
    ...
    {if (($smarty.request.view eq 'Edit') or ($MODULE_NAME eq 'Webforms')) && !in_array($REFERENCE_LIST[0],$QUICKCREATE_RESTRICTED_MODULES)}
            <button type="button" name="create"  class="createReferenceRecord  btn btn-default  clearfix" title="{vtranslate('LBL_CREATE', $MODULE)}">
