...
{strip}
	...
	<select class="select2 inputElement" name="{$FIELD_MODEL->getFieldName()}">
		{foreach item=CURRENCY_NAME key=CURRENCY_ID from=$CURRENCY_LIST}
			<option value="{$CURRENCY_ID}" data-picklistvalue= '{$CURRENCY_ID}' {if $FIELD_MODEL->get('fieldvalue') eq $CURRENCY_ID} selected {/if}>{vtranslate($CURRENCY_NAME, 'Vtiger')}</option>
		{/foreach}
	</select>
{/strip}