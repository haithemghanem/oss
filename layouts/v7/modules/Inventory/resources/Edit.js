...
Vtiger_Edit_Js("Inventory_Edit_Js", {
    ...
    lineItemPopOverTemplate : '<div class="popover lineItemPopover" role="tooltip"><div class="arrow"></div>\n\
								...
								<div class="popover-content"></div>\n\
									<div class="modal-footer lineItemPopupModalFooter">\n\
										<center>\n\
										<button class="btn btn-success popoverButton" type="button"><strong>حفظ</strong></button>\n\
										<a href="#" class="popoverCancel" type="reset">الغاء</a>\n\
										</center>\n\
									</div>\n\
                                </div>'
    ...
    initializeVariables : function() {
        ...
        this.aftTaxTotalEle = jQuery('#aftTaxTotal');
        ...
        this.storagevalue = jQuery('#storagevalue');
        this.deliveryvalue =jQuery('#deliveryvalue');

        //hdnS_H_Percent,hdnS_H_Amount
    },
	...
	getstoragevalueValue : function() {
		return parseFloat(this.storagevalue.val());
	},
	getdeliveryvalueValue : function() {
		return parseFloat(this.deliveryvalue.val());
	},
	...
	setAfterTaxTotal : function(valu =0){
      this.aftTaxTotalEle.text(valu);
      return this;
	},
...
//		this.calculateGrandTotal();
    ...
    calculateGrandTotal : function(){
		...
		var storagevalue = this.getstoragevalueValue();
		var deliveryvalue = this.getdeliveryvalueValue();

        var afterTaxTotal = parseFloat(netTotal) - parseFloat(discountTotal) + parseFloat(shippingHandlingCharge) + parseFloat(shippingHandlingTax) - parseFloat(deductedTaxesAmount);
		var grandTotal = parseFloat(netTotal) - parseFloat(discountTotal) + parseFloat(shippingHandlingCharge) + parseFloat(shippingHandlingTax) - parseFloat(deductedTaxesAmount) + parseFloat(storagevalue) + parseFloat(deliveryvalue);

		if(this.isGroupTaxMode()){
			grandTotal +=  this.getGroupTaxTotal();
			afterTaxTotal +=  this.getGroupTaxTotal();
		}
       ...
       //aftTaxTotalEle
		...
		this.setAfterTaxTotal(afterTaxTotal);
	},
    ...
    gtpricefunction:function(recordid){
    	var custmerid = "";
    	custmerid = jQuery('input[name="contact_id"]').val();
    	var cuprice =0;
    	...
    	//console.log("custmerid if  ="+custmerid);
           	 var params = {
					'module' : 'Invoice',
					'view' : 'GetlistPrice',
					'serviceid'  : recordid,
					'contactsid'  : custmerid
				};
				...
				if(result.sucess){
					...
					cuprice = result.price;
					//console.log("respons price = "+result.sucess);
				    //console.log("respons price value = "+result.price);
				    return 0;
				}
			});
    ...
    mapResultsToFields: function(parentRow,responseData){
        ...
        //haithem
    	//console.log("Fill Prodect ="+responseData);
		...
		for(var id in responseData){
           ...
           //console.log("cuprice   ="+cuprice);
            ...
            //jQuery('input.listPrice',parentRow).val(900); contact_id
            var custmerid = "";
    	    custmerid = jQuery('input[name="cf_944"]').val();
    	     ...
    	     //console.log("custmerid if  ="+custmerid);
				...
				if(result.sucess){
					//console.log("respons price = "+result.sucess);
				    //console.log("respons price value = "+result.price);
				    jQuery('input.listPrice',parentRow).val(result.price);
    ...
    registerAddProductService : function() {
        ...
        var addLineItemEventHandler = function(e, data){
            ...
            if(typeof data != "undefined") {
             ...
             newLineItem.find('input.productName').focus();
        }
    ...
    registerProductAndServiceSelector : function() {
        ...
        this.lineItemsHolder.on('click','.lineItemPopup', function(e){
            ...
            var postPopupHandler = function(e, data){
               //console.log("responsa=" + JSON.stringify(data));
	 ...
	 registerPriceBookPopUp : function () {
		...
		this.lineItemsHolder.on('click','.priceBookPopup',function(e){
            ...
            var postPriceBookPopupHandler = function(e, data) {
                ...
                //console.log("responseData=" + JSON.stringify(e));
...
//						}
    ...
    registerChargeBlockShowEvent : function(){
		...
		chargesTrigger.on('shown.bs.popover', function(){
			...
			jQuery('#chargesBlock').find('.chargeName').text("تكاليف اخرى");
	...
	registerstoragevalueChange : function() {
		//console.log("hi Haitme registerstoragevalueChange");
		var self = this;
		this.storagevalue.on('focusout',function(e){
            var element = jQuery(e.currentTarget);
            if(self.getForm().data('validator').element(element)) {
                var value = element.val();
                if(value == ""){
	...
	registerdeliveryvalueChange : function() {
		//console.log("hi Haitme register deliveryvalue Change");
		var self = this;
		this.deliveryvalue.on('focusout',function(e){
            var element = jQuery(e.currentTarget);
            if(self.getForm().data('validator').element(element)) {
                var value = element.val();
                if(value == ""){
	...
	registerchargesTotalChange : function() {
		//console.log("hi Haitme register thtal change Change");
		//console.log("Test vale :=" +this.chargesTotalEle.val());
		//console.log("Test vale :=" +this.chargesTotalEle.val());
		//chargeValue

		var self = this;
		
		jQuery('#discountTotal_final_input').on('focusout',function(e){
            var element = jQuery(e.currentTarget);
            if(self.getForm().data('validator').element(element)) {
                var value = element.val();
                if(value == ""){
                ...
                jQuery('#discount_amount_final').val(element.val());
                jQuery('#discountTotal_final').text(element.val());
                jQuery('#overallDiscount').text('('+element.val()+')');
                
                self.calculatePreTaxTotal();
		...
		this.chargesTotalEle.on('focusout',function(e){
            var element = jQuery(e.currentTarget);
            if(self.getForm().data('validator').element(element)) {
                var value = element.val();
                if(value == ""){
                ...
                //console.log("charges total ele="+element.val())

                self.chargesTotalEle.val(parseFloat(element.val()));
				self.chargesTotalDisplay.text(parseFloat(element.val()));
				jQuery('.chargeValue').val(element.val());
				//jQuery('#chargeTaxTotal').text(parseFloat(element.val()));
				//self.chargeTaxesTotal.val(parseFloat(element.val()));
				//.toFixed(this.numOfCurrencyDecimals));
    ...
    registerLineItemEvents : function() {
        ...
        this.registerstoragevalueChange();

        this.registerdeliveryvalueChange();
	    this.registerchargesTotalChange();
				...
				self.searchModuleNames(params).then(function(data){
					...
					if(serverDataFormat.length <= 0) {
						serverDataFormat = new Array({
							'label' : app.vtranslate('لايوجد تطابق'),
							'type'  : 'no results'
						});
					}
...
//			};
	...
	registerReferenceSelectionEvent : function(container) {
		...
		jQuery('input[name="contact_id"]', container).on(Vtiger_Edit_Js.referenceSelectionEvent, function(e, data){
			...
			var sourceModu = data['source_module'];
       	    var selnaem= data['selectedName']; 
       	    ...
       	    var params = {
				...
				var result = JSON.parse(response);
				if(result.success){
                  jQuery("#Invoice_editView_fieldName_customerno").val(result.phone);
				}
    ...
    referenceDeSelectionEvent : function (container){
    	//console.log("start cf_944-888-jjj- id referenceSelectionEven");
       jQuery('input[name="cf_944"]', container).on(Vtiger_Edit_Js.referenceSelectionEvent, function(e, data){
       	    var sourceModu = data['source_module'];
       	    var selnaem= data['selectedName']; 
       	    var record = data['record'];
       	    var params = {
				...
				var result = JSON.parse(response);
				if(result.success){
                  jQuery("#Invoice_editView_fieldName_cf_946").val(result.phone);
                  jQuery("#Invoice_editView_fieldName_cf_966").val(result.idnumber);
                  jQuery("#Invoice_editView_fieldName_cf_976").val(result.city);
                  jQuery("#Invoice_editView_fieldName_cf_978").val(result.address);
                  
                  
                  var bookpri="0";
                  bookpri=result.bookprice;
                
                  if(bookpri  !=0 && bookpri !="0"){
                      //background: #4CAF50;
                      jQuery("#cf_944_display").css("background" , "#4CAF50");
                  }else{
                      
                       jQuery("#cf_944_display").css("background" , "#e3e3e3");
                  }
                  
                  //city ,address , idnumber , bookprice
				}
			...
			//console.log("hi event cf_944 id referenceSelectionEven="+data['source_module']+ "--val");
    ...
    registerBasicEvents: function(container){
        ...
        this.referenceDeSelectionEvent(this.getForm());
