...
<div name='editContent'>
	{if $DUPLICATE_RECORDS}
     ...
     {else}
          {if $DUPLICATE_RECORDS1}
          <div class="fieldBlockContainer duplicationMessageContainer">
            <div class="duplicationMessageHeader"><b style="color:red;">{$DUPLICATE_RECORDS1}</b></div>
          </div>
         {/if}
	{/if}
    ...
    {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name=blockIterator}
        {if $BLOCK_LABEL eq 'LBL_ITEM_DETAILS'}{continue}{/if}
         {if $BLOCK_FIELDS|@count gt 0}
             <div class='fieldBlockContainer col-lg-6 col-md-12 {$BLOCK_LABEL}' data-block="{$BLOCK_LABEL}">
