...
<div name='editContent'>
	...
	{if $BLOCK_FIELDS|@count gt 0}
		...
		<div class='fieldBlockContainer col-lg-7 col-md-12 prodectmainitem '>
						...
						<select class="select2" id="currency_id" name="currency_id" style="width: 150px;">
							{foreach item=currency_details key=count from=$CURRENCIES}
								<option value="{$currency_details.curid}" class="textShadowNone" data-conversion-rate="{$currency_details.conversionrate}" {if $SELECTED_CURRENCY.currency_id eq $currency_details.curid} selected {/if}>
									...
									{vtranslate($currency_details.currencylabel,'vtiger')} ({$currency_details.currencysymbol})
								</option>
							{/foreach}
						</select>
			...
			<div class="lineitemTableContainer" >
				<table class="table table-bordered" id="lineItemTab">
					<tr>
						...
						{/if}
						<td style="display: none;"><strong class="pull-right">{vtranslate('LBL_TOTAL',$MODULE)}</strong></td>

						{if $MARGIN_EDITABLE && $PURCHASE_COST_EDITABLE}
						  <td style="display: none;" class="haithemghanem">
		...
		<div class="fieldBlockContainer col-lg-4 col-md-12 prodectmainfooter">
			<table class="table table-bordered blockContainer lineItemTable" id="lineItemResult">
				...
				{if $DISCOUNT_AMOUNT_EDITABLE || $DISCOUNT_PERCENT_EDITABLE}
					<tr>
						...
						<td>
						   ...
						   <input type="text" class="lineItemInputBox form-control" id="discountTotal_final_input" name="discount_amount_final" value="{if $FINAL.discountTotal_final}{$FINAL.discountTotal_final}{else}0{/if}" />
							...
							<div id="finalDiscountUI" class="finalDiscountUI validCheck hide">
								...
								<table width="100%" border="0" cellpadding="5" cellspacing="0" class="table table-nobordered popupTable">
									<tbody>
										<tr class="hide">
											<td><input type="radio" name="discount_final" class="finalDiscounts" data-discount-type="zero" {if $DISCOUNT_TYPE_FINAL eq 'zero'}{/if} />&nbsp; {vtranslate('LBL_ZERO_DISCOUNT',$MODULE)}</td>
											<td class="lineOnTop">
										...
										{if $DISCOUNT_PERCENT_EDITABLE}
											<tr class="hide">
												...
												<td><span class="pull-right">&nbsp;%</span><input type="text" data-rule-positive=false data-rule-inventory_percentage=true id="discount_percentage_final" name="discount_percentage_final" value="{$FINAL.discount_percentage_final}" class="discount_percentage_final span1 pull-right discountVal {if $DISCOUNT_TYPE_FINAL neq 'percentage'}hide{/if}" /></td>
											</tr>
										{/if}
										{if $DISCOUNT_AMOUNT_EDITABLE}
											<tr>
												<td><input type="radio" name="discount_final" class="finalDiscounts" data-discount-type="amount" {if $DISCOUNT_TYPE_FINAL eq 'amount'}{/if} checked />&nbsp;{vtranslate('LBL_DIRECT_PRICE_REDUCTION',$MODULE)}</td>
												<td><input type="text" data-rule-positive=false id="discount_amount_final" name="discount_amount_final" value="{$FINAL.discount_amount_final}" class="span1 pull-right discount_amount_final discountVal {if $DISCOUNT_TYPE_FINAL neq 'amount'}{/if}" /></td>
											</tr>
										{/if}
									</tbody>
								</table>
				...
				{if $SH_PERCENT_EDITABLE}
					...
					<tr class="hide">
						...
						<td>
							<input type="text" class="lineItemInputBox" id="chargesTotal" name="shipping_handling_charge" value="{if $FINAL.shipping_handling_charge}{$FINAL.shipping_handling_charge}{else}0{/if}" />
				...
				<tr valign="top" class="hide">
						<td width="50%" >
							<div class="pull-right">
								<strong>{vtranslate('Total After Tax',$MODULE)}</strong>
							</div>
						</td>
						<td>
							    {assign var=PRE_T_TOTAL value=$FINAL.preTaxTotal + $FINAL.tax_totalamount}
							    {assign var=T_TOTAL value=$FINAL.tax_totalamount}

						        <span class="pull-right" id="aftTaxTotal">{if $PRE_T_TOTAL}{$PRE_T_TOTAL}{else}0{/if}</span>
						        <input type="hidden" id="aft_Tax_Total" name="aft_Tax_Total" value="{if $PRE_T_TOTAL}{$PRE_T_TOTAL}{else}0{/if}"/>
						</td>
				  </tr>
				 ...
				 <tr valign="top" class="hide">
						<td width="50%" >
							<div class="pull-right">
								{if $MODULE eq 'Invoice'}
									<strong>{vtranslate('Storage Value',$MODULE)}</strong>
								{else}
									<strong>{vtranslate('Storage Value',$MODULE)}</strong>
								{/if}
							</div>
						...
						<td>
							{if $MODULE eq 'Invoice'}
								<span class="pull-right"><input id="storagevalue" name="storagevalue" type="text" class="lineItemInputBox form-control" value="{if $RECORD->getDisplayValue('storagevalue') && !($IS_DUPLICATE)}{$RECORD->getDisplayValue('storagevalue')}{else}0{/if}"></span>
							{/if}
						</td>
					</tr>
					 ...
					 <tr valign="top" class="hide">
						<td width="50%" >
							<div class="pull-right">
								{if $MODULE eq 'Invoice'}
									<strong>{vtranslate('Delivery value',$MODULE)}</strong>
								{else}
									<strong>{vtranslate('Delivery value',$MODULE)}</strong>
								{/if}
							</div>
						...
						<td>
							{if $MODULE eq 'Invoice'}
								<span class="pull-right"><input id="deliveryvalue" name="deliveryvalue" type="text" class="lineItemInputBox form-control" value="{if $RECORD->getDisplayValue('deliveryvalue') && !($IS_DUPLICATE)}{$RECORD->getDisplayValue('deliveryvalue')}{else}0{/if}"></span>
							{/if}
						</td>
					</tr>
