...
{strip}
	...
	{if $PRODUCT_EDITABLE}
		<td>
			...
			{else}
				{if $COMMENT_EDITABLE}
					<div><input  style="min-width: 280px; margin-top: 3px;" type="text"  id="{$comment}" name="{$comment}" class="lineItemCommentBox" value="{decode_html($data.$comment)}" />
					</div>
				{/if}
			{/if}
		</td>
	...
	<td style="max-width: 50px;">
		<input id="{$qty}" style="width: 100%;" name="{$qty}" type="text" class="qty smallInputBox inputElement"
	...
	{if $LIST_PRICE_EDITABLE}
		<td style="max-width: 60px;">
			<div>
				<input style="width:100%" id="{$listPrice}" name="{$listPrice}" value="{if !empty($data.$listPrice)}{$data.$listPrice}{else}0{/if}" type="text"
				...
				{if $PRICEBOOK_MODULE_MODEL->isPermitted('DetailView') && $MODULE != 'PurchaseOrder'}
					<span class="priceBookPopup cursorPointer ietepriceBookPopup hide" data-popup="Popup"  title="{vtranslate('PriceBooks', $MODULE)}" data-module-name="PriceBooks" style="float:left">{Vtiger_Module_Model::getModuleIconPath('PriceBooks')}</span>
				{/if}
			</div>
...
jQuery("#Invoice_editView_fieldName_cf_987").focus();
jQuery("#cf_944_display").on("focusout", function(e){
  // $("#Invoice_editView_fieldName_subject").focus();
});

jQuery("#Invoice_editView_fieldName_subject").on("focusout", function(e){
    jQuery("#Invoice_editView_fieldName_cf_952").focus();
});

jQuery("#Invoice_editView_fieldName_cf_952").on("focusout", function(e){
    $("#productName1").focus();
});

jQuery("#charges").on("focusout", function(e){
     $("#chargesTotal").select();
});
