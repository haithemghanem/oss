...
Vtiger.Class("Calendar_Calendar_Js", {
	...
	registerFeedChangeEvent: function () { 
		var thisInstance = this;
		//start check all by haithem
		jQuery('#checkalluser').on('click',function(){
			 var curentTarget = jQuery(this);
			 if (curentTarget.is(':checked')) {
			    jQuery('input[type="checkbox"].toggleCalendarFeed').each(function(){
			    var curentTar = jQuery(this);
			    var sourceKey = curentTar.data('calendarSourcekey');
			    if(sourceKey =='undefined' || sourceKey ==null || sourceKey==''){
			    	
			    }else{
			       if(curentTar.is(':checked')){
			       }else{
			    	curentTar.attr('checked','checked');
			    	thisInstance.enableFeed(sourceKey);
			    	thisInstance.addEvents(curentTar);
			    	 }
			        }
			    });
			  }else{
			    ...
			    if(sourceKey =='undefined' || sourceKey ==null || sourceKey==''){
			    	...
			    	//console.log("");
			    	if(curentTar.is(':checked')){
			    		curentTar.removeAttr('checked');
			    	    thisInstance.disableFeed(sourceKey);
					    thisInstance.removeEvents(curentTar);
